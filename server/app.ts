...
import { db } from './db';
import { parseReceiptWithGemini, sampleReceiptTemplates } from './ocr';
...
export default app;
  ...
  app.patch('/api/expenses/:id', (req, res) => {
    const user = db.getCurrentUser();
    const {
      groupId, title, description, amount, category, date, paidBy,
      source, splitMethod, items, participants, receiptUrl
    } = req.body;

    if (!title || !title.trim()) return res.status(400).json({ error: 'Expense title is required' });
    if (!amount || Number(amount) <= 0) return res.status(400).json({ error: 'Valid expense amount is required' });
    if (!participants || participants.length === 0) return res.status(400).json({ error: 'At least one participant is required' });

    const updated = db.updateExpense(req.params.id, user.id, {
      groupId, title, description, amount: Number(amount), category: category || 'Food',
      date, paidBy: paidBy || user.id, source: source || 'manual', splitMethod: splitMethod || 'equal',
      items, participants, receiptUrl
    });

    if (!updated) {
      return res.status(403).json({ error: 'Unable to update expense. Only creator or payer can edit.' });
    }
