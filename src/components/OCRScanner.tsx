...
export const OCRScanner: React.FC<OCRScannerProps> = ({ onComplete, onCancel, defaultGroupId }) => {
  ...
  // Rotate image by 90 degrees for optimal OCR parsing angle
  const rotateImage = (degrees: number = 90) => {
    ...
    img.onload = () => {
      ...
      showToast(`Rotated receipt ${degrees}° for optimal OCR reading`, 'info');
    };
      ...
      {step === 'preview_capture' && selectedImage && (
        <div className="space-y-4">
          <div className="relative rounded-2xl overflow-hidden bg-slate-950 aspect-[4/3] sm:aspect-[16/10] flex items-center justify-center border border-slate-800">
            ...
            {/* Quick Rotate Button on Image */}
            <button
              type="button"
              onClick={() => rotateImage(90)}
              className="absolute top-3 right-3 bg-black/60 hover:bg-black/80 backdrop-blur-md text-white px-3 py-1.5 rounded-xl text-xs font-semibold flex items-center gap-1.5 transition-colors cursor-pointer"
          ...
          <div className="flex items-center justify-between gap-3 pt-2">
            <div className="flex items-center gap-2">
              <button
                type="button"
                onClick={() => startCamera()}
                className="px-3.5 py-2.5 rounded-xl border border-slate-200 dark:border-slate-700 text-xs font-semibold text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800 flex items-center gap-1.5 cursor-pointer"
              >
